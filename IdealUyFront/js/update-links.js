document.addEventListener("DOMContentLoaded", () => {
    // Adding product ID to "Batafsil ko'rish" links on index page
    const detailLinks = document.querySelectorAll('.product-card a[href="product-detail.html"]')

    detailLinks.forEach((link) => {
        // Finding the parent product card
        const productCard = link.closest(".product-card")
        if (productCard) {
            // Finding the add to cart button to get the product ID
    ...
    // Also checking for links in featured products section
